...
<link rel="stylesheet" href="/ZsGljSystem/styles/Admin/News/jquery-ui.css">
...
<!-- 类别+下拉框 -->
		...
		<select id="queryTypeName" style="width:10%;min-width: 75px;font-size:13px;">
			...
			<option value=2>公路局</option>
			<option value=3>管养单位</option>
		</select> 
...
<!-- 名称+Input框 -->
		...
		<input type="text" style=" width:11% ;min-width: 140px; font-size:13px;" id="queryUserName"> 
...
<!-- 添加按钮 -->
		<input type="<%=request.getAttribute("addBtnType")%>" id="add" src="/ZsGljSystem/icons/Admin/News/add.png" 
					style="  width:62px;height:30px; float:right;margin-left:5px;padding-bottom: 10px;" onclick="location.href='/ZsGljSystem/actionsUser/putupdateUser.action'" />
	</div>
	...
	<div style="margin-top: 20px;margin-right: 22px;">
		<table id="dg" class="easyui-datagrid"
			...
			url="/ZsGljSystem/actionsUser/userlistData" rownumbers="false"
			fitColumns="true" singleSelect="true" pagination="true" method="post"
			pagination="true" collapsible="true">
			<thead>
				<tr>
					<!-- th field="userID" style="width:10%;">用户ID</th> -->
					<th field="userName" style="width:10%;">用户名称</th>
					<th field="userType" style="width:10%;" formatter = "formatUserType">类别</th>
					<th field="companyID" style="width:15%;">所属单位</th>
					<th field="telephone" style="width:10%;">联系电话</th>
					<th field="status" style="width:10%;" formatter="formatUserStatus">状态</th>
					<th field="remark" style="width:20%;">备注</th>
	...
	function formatUserType(value, rowData, rowIndex) {
		var result;
		if (value == null) {
			return null;
		} else if (value == "1") {
	...
	function formatOper(val, row, index) {
		...
		{
			if(checkstatus != 0){
				return '<input type="image" style="margin-left:3px;" id="view" src="/ZsGljSystem/icons/Admin/News/view.png" onmousedown="viewMouseDown(this)" onmouseover="viewMouseOver(this)" onmouseout="viewMouseOut(this)" onclick="showUsers('
				+ index
				+ ')" />'
				+ '<input type="image" style="margin-left:3px;" id="update" src="/ZsGljSystem/icons/Admin/News/update.png" onmousedown="updateMouseDown(this)" onmouseover="updateMouseOver(this)" onmouseout="updateMouseOut(this)" onclick="updateUsers('
				+ index
				+ ')"/>'
				+ '<input type="image" style="margin-left:3px;" id="delete" src="/ZsGljSystem/icons/Admin/News/delete.png" onmousedown="deleteMouseDown(this)" onmouseover="deleteMouseOver(this)" onmouseout="deleteMouseOut(this)" onclick="deleteUsers('
				+ index
				+ ')"/>';
			}
			else{
	...
	//查看用户
	function showUsers(index,obj) {
		$('#dg').datagrid('selectRow', index);
		var row = $('#dg').datagrid('getSelected');
		var userId = row.userID;
		//alert(userId);
		var status = row.status;
		location.href="/ZsGljSystem/actionsUser/showUser.action?userId="+userId;
	}
	
	//更新用户
	function updateUsers(index,obj) {
		$('#dg').datagrid('selectRow', index);
		var row = $('#dg').datagrid('getSelected');
		var userId = row.userID;
		//alert(userId);
		var status = row.status;
		location.href="/ZsGljSystem/actionsUser/putupdateUser.action?userId="+userId;
	}
	
	//删除用户
	function deleteUsers(index,obj) {
		$('#dg').datagrid('selectRow', index);
		var row = $('#dg').datagrid('getSelected');
		var string = row.userName;
		string = encodeURI(encodeURI(string));
		if (row) {
			$.messager.defaults = { ok: "是", cancel: "否" };
			$.messager.confirm('Confirm', '确定删除用户' + row.userName + '?', function(r) {
				if (r) {
					var url = "/ZsGljSystem/actionsUser/deleteUser.action?" + "userName=" + string;
					$.ajax({
						type : "post",
						url : url,
						cache : false,
	...
	//按照查询条件进行查询
	function query(){
		var userTypevalue = $("#queryTypeName option:selected").val();
		var status =  $('#queryStatus').val();
		var queryUserName = $('#queryUserName').val();
		//防止乱码
		queryUserName = encodeURI(queryUserName);
		var url = "/ZsGljSystem/actionsUser/queryuserlist.action?" +"&queryStatus="+status+"&userTypevalue="+userTypevalue+"&queryUserName="+queryUserName;
			$.ajax({
				type : "post",
				url : url,
				cache : false,
